...
Chests = (function() {
  ...
  Chests.prototype.activate = function(url, interrupt) {
    ...
    if (interrupt && this.drawer.drawers.length > 0) {
      this.log("\n ---- Interrupted: all promises will be rejected :: " + ((_ref = (_ref1 = this.url.prev) != null ? _ref1.url : void 0) != null ? _ref : null) + "----\n");
      this.drawer.clear();
    }
    this.url = new Url({
      prev: this.url || null,
    ...
    return this.drawer.then((function(_this) {
      return function() {
...
},{"./drawer.coffee":3,"es6-promise":4,"underscore":15}],3:[function(require,module,exports){
...
},{"es6-promise":4,"underscore":15}],4:[function(require,module,exports){
"use strict";
...
describe("urlの解析", function() {
  return it("parseUrl", function() {
    expect(uu._parseUrl("/").length).to.be(1);
    expect(uu._parseUrl("/user").length).to.be(2);
    expect(uu._parseUrl("user/index").length).to.be(3);
    expect(uu._parseUrl("/user/index").length).to.be(3);
    expect(uu._parseUrl("user/index/").length).to.be(3);
    return expect(uu._parseUrl("/user/index/").length).to.be(3);
  });
});

describe("1回目のアクセス", function() {
  return it("findRoute: 2つのURLから通過部分をみつける", function() {
    expect(uu.findRoute("/", null)).to.be.an(Array);
    expect(uu.findRoute("/", null)[1]).to.contain("/");
    return expect(uu.findRoute("/news/id", null)[1]).to.contain("/news");
  });
});

describe("pushStateなどでのページ遷移時", function() {
  return it("findRoute: 2つのURLから通過部分をみつける", function() {
    expect(uu.findRoute("/", "/user/")).to.be.an(Array);
    expect(uu.findRoute("/user/edit", "/user/detail/id")).to.be.an(Array);
    return expect(uu.findRoute("/user/edit/id", "/user/detail/id")).to.be.an(Array);
  });
});

describe("resolvePath", function() {
  return it("パス配列に接頭辞をつける", function() {
    expect(uu.resolvePath(["/"], ["detail", "id"])).to.be.contain("/detail", "/detail/id");
    return expect(uu.resolvePath(["/user"], ["detail", "id"])).to.be.contain("/user/detail", "/user/detail/id");
  });
});

describe("splitPath", function() {
  return it("遷移前、遷移後のURLを共通部分、それ以外に分ける", function() {
    var common, down, up, _ref;
    _ref = uu.splitPath("/user/edit/id", "/user/detail/id"), up = _ref[0], down = _ref[1], common = _ref[2];
    _.tap(expect(up).to.be, function(m) {
      m.contain("detail");
      return m.contain("id");
    });
    return _.tap(expect(down).to.be, function(m) {
      m.contain("edit");
      return m.contain("id");
    });
  });
...
describe("findRoute", function() {
  return it("遷移時に通過するルートを見つける", function() {
    var down, up, _ref, _ref1;
    _ref = uu.findRoute("/user/edit/id", "/user/detail/id"), up = _ref[0], down = _ref[1];
    expect(up).to.have.length(2);
    expect(down).to.have.length(2);
    _ref1 = uu.findRoute("/user/edit", "/user/detail/id"), up = _ref1[0], down = _ref1[1];
    expect(up).to.have.length(2);
    return expect(down).to.have.length(1);
  });
});

o = null;

describe("URLパターンを登録する", function() {
  it("パラメータ受け取りのパターン", function() {
    var obj;
    uu.on("/users/edit/:id-:page", "open", function() {
    ...
    obj = (function() {
      ...
      obj.prototype.enter = function() {
        return this.ed = wait(this.wait);
      };

      obj.prototype.open = function() {
        return this.od = wait(this.wait);
      };

      obj.prototype.leave = function() {
        if (this.ed) {
          return this.ed.reject();
        }
      };

      obj.prototype.close = function() {
        if (this.od) {
          return this.od.reject();
        }
      };
    ...
    return uu.on("/users/detail/:id|:opt", new obj);
  });
  return it("遷移中に別のURLへ移動するパターン", function(done) {
    uu.activate("/users");
    return wait(20).then(function() {
      return uu.activate("/users/edit/1", true);
    }).then(function() {
      return uu.activate("/users", true);
    }).then(function() {
      return done();
    });
  });
...
describe("off", function() {
  it("コールバックを指定して削除", function() {
    var len;
    len = uu.routes.length;
    uu.off(o.enter);
    return expect(uu.routes).to.have.length(len - 1);
  });
  it("URLとイベントを指定して削除", function() {
    var len;
    len = uu.routes.length;
    uu.off("/users", "open");
    return expect(uu.routes).to.have.length(len - 1);
  });
  return it("引数なしで呼び出すとエラー", function() {
    return expect(uu.off).to.throwError(/Invalid arguments./);
  });
});

describe("Promiseのテスト", function() {
  ...
  return it("途中でReject", function(done) {
    var p;
    p = wait(100);
    p["catch"](function() {
      ...
      return console.log("rejected 3");
    }).then(function() {
      console.log("finished");
      return done();
    });
    return wait(70).then(function() {
      return p.reject("interrupt");
    });
  });
});


},{"../lib/chests.coffee":2,"es6-promise":4,"jquery":14,"underscore":15}]},{},[16])